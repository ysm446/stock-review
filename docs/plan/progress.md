...
- **2026-07-19: 株価チャートに信用残（信用買い残・売り残）を追加**。
  ...
  - **同日追記2（過去分バックフィル）**: Wayback Machine のCDX APIでJPX週次PDFのアーカイブを発見（200応答92週: 2015〜16 / 2021〜22 / 2024〜25、不連続）。2015年・2021年の旧フォーマットPDFも既存パーサ＋恒等式検証でそのまま解析できることを確認し、`fetch_margin.py --backfill` を実装（未取り込み週のみ・週ごとにコミットで再実行安全・1秒スリープ）。隔離DATA_DIRで2016/2024の2週スモークテスト後、実データ（data-2026-7-7）へ直近5週＋バックフィルを実行。欠落期間（2017〜2020・2023・2025秋〜2026春）はJ-Quants Standard（有料）でしか埋められない。
  - **同日追記13（ノートカードのグリッド化＋旧ノート削除）**: ノートカードをニュースパネル同様の行列表示に変更（`.review-notes-body` を `repeat(auto-fill, minmax(250px, 1fr))` のグリッドに、フォントも小型化）。「旧ノートはすべて削除して大丈夫」との指示を受け、legacy表示機能（store/API/フロントの legacy 分岐と `.is-legacy`）と旧単一ノート関数（`get/save/restore_stock_notes`・`stock_notes_path`）をコードごと削除し、実データの `notes.md`/`notes.md.bak` 23ファイル（16銘柄）も削除。ストアテスト6項目再パス・グリッド表示をハーネス目視確認。
  - **同日追記12（ノートのカテゴリー別カード分割）**: ノートを7カテゴリー（business/financials/recent/history/environment/thesis/misc、定義は `chat_store.NOTE_CATEGORIES`）のカードに分割。保存は `stocks/<ticker>/notes/<key>.md`＋カード単位 `.md.bak`。API は `GET /stocks/{t}/notes`（cards+legacy）/`PATCH /stocks/{t}/notes/{key}`/`POST .../restore` に刷新（旧単一ノートAPIは廃止、workspace エンドポイントも cards 形式）。「ノートに反映」は2段階: ①分類器プロンプトで反映先key（1〜3個、無関係なら none）をLLMに選ばせる → ②選ばれたカードごとに現内容＋会話でマージし PATCH（出力が同一なら保存しない）。「ノートを作り直す」はカテゴリー見出し付き全文を生成し `splitRegeneratedNote` で分割保存（未知見出しは misc、既知見出しゼロならエラー）。旧 notes.md は「旧ノート（分割前）」として読み取り専用表示（legacy）。プロンプト設計は `docs/design/stock-review-note-prompt.md` を全面更新。検証: 隔離DATA_DIRでストア7項目（初期/保存/往復restore/同一内容でbak温存/不明キー400/legacy/バックアップ無しrestore）、分割ロジックのNode単体（既知/未知見出し・フェンス・見出し無し）、実CSSハーネスでカードUI目視。実LLMでのルーティング・マージ品質は未確認（運用でのチューニング前提）。
  - **同日追記11（指標カードの余白削減）**: `.review-lower-scroll` を `container-type: inline-size` にし、`@container (min-width: 860px)` で `.review-grid-layout` を3カラム＋財務サマリー `span 2` に（狭いときは従来の2カラム）。後方の `.review-financial-panel { grid-column: auto }` が span 2 を打ち消していたため削除。`.review-kv-row` のラベル列は固定110pxに変更。数値は一度左揃え（ラベル直後）にしたが「カード右側が空く」との指摘で**右端揃えを維持**（3カラム化でカードが狭くなり間延びは解消済み）。実CSSハーネス（1400px窓、コンテナ950px/640px）で両配置を目視確認。≤1180pxのメディアクエリ（2カラム強制）はコンテナ幅860px未満となるため実質衝突しない。
  - **同日追記10（Waybackバックフィル機能の削除）**: ユーザー判断で Internet Archive バックフィルを機能ごと削除（`iter_backfill`/`backfill_from_wayback`/`normalize_since`/CLI `--backfill --since`/`POST /margin/backfill`/設定画面の期間選択＋取り込みUI）。自動取り込みトグル（`/margin/settings`）と14日ルールの欠測分断描画は残置。実データに取り込み済みの2024年以降18週分も残置（削除はしていない）。復活時はコミット `0aee1f6`〜`7adfe03` を参照。
  - **同日追記9（レビュー画面レイアウト崩れの修正）**: 追記8のハンドル（grid-column:1明示）が、自動配置だった `.review-metrics-col` / `.stock-chat-panel` を押し出してレイアウトが崩れていた（明示配置アイテムが先に置かれ、自動配置が空きセルへ流れるため）。両カラムに `grid-column`/`grid-row` を明示して修正。≤1500pxの1カラムフォールバックでは `grid-row: auto` に戻して縦積みを維持。実styles.cssを読むハーネス（1600px幅）で2カラム＋ハンドル表示、1280px幅で1カラム縦積みを目視確認。
  - **同日追記8（レビュー画面の境界ドラッグ＋共通化）**: カラム境界ドラッグを `src/grid-resizer.js` の `setupColumnResizer({grid, handle, cssVar, storageKey, min, max})` へ共通化し、renderer-market.js のインライン実装を置き換え。レビュー画面は `.review-main-grid` の `--review-split`（既定52.5% ≒ 1.05fr:0.95fr、範囲32〜72%）＋ `#review-col-resizer` で指標列/銘柄チャット列の幅を調整（`stock-review.reviewSplitX`）。≤1500pxの1カラムフォールバックではハンドルを非表示。実画面でのドラッグ操作は未確認。
  - **同日追記7（自動取り込みトグル＋マーケット境界ドラッグ）**: (1) 信用残の自動取り込みを設定 > データのチェックボックスでオン・オフ化。設定は `margin_meta.auto_ingest`（DB側、既定オン）に保存し `GET/PUT /margin/settings` で読み書き。`ingest_safely` が設定を確認してオフなら即return（JPXアクセスなし）。隔離DATA_DIRでオフ時未実行→オン時実行を確認。(2) マーケットページ: 左右分割は `.market-grid` の `--market-split`（%、既定63・範囲35〜80）を `#market-col-resizer`（grid列1のjustify-self:endでgap上に配置）のドラッグで変更し `stock-review.marketSplitX` に保存、ダブルクリックでリセット。上下（チャート/まとめ）は既存の `createCandlestickChart` resizer機構を市場チャートに配線（`#market-chart-resizer`、`stock-review.marketChartHeight`、既定320px）。実画面でのドラッグ操作は未確認。
  - **同日追記6（UI操作の改善）**: (1) `button:focus { outline:none }` + `button:focus-visible` の控えめなリングを追加し、クリック後にOSアクセント色のフォーカスリングが残る問題を解消（既存の input/select の outline 抑制と同じ方針）。(2) `createCandlestickChart` に `arrowKeyScrub()` config を追加し、レビュー画面表示中に ←→ キーで `setEndOffset(±1)`（1日戻る/進む）。INPUT/TEXTAREA/SELECT/contentEditable フォーカス中・修飾キー押下・`.modal-backdrop:not(.is-hidden)` 表示中は無効。マーケットチャートには未配線（個別チャートのみの要望のため）。実画面でのキー操作は未確認。
  - **同日追記5（折れ線化）**: 階段状より傾向を追いやすいとの要望で、信用残の描画を「週次点（申込日の足の位置にアンカー、同日の足が無ければ14日以内の直近過去の足）を直線で結ぶ折れ線＋各点にドット（半径2）」へ変更。14日超の間隔は結ばない（欠測分断は維持）。孤立週はドットのみ表示される。`getMarginValues`（14日ルールの割当）はツールチップ用に残置。ハーネス（欠測＋孤立1週の合成データ）で目視確認済み。
  - **同日追記4（欠測区間の表示修正）**: アーカイブ欠落期間が「最後の値のフラット線」で埋まり誤解を招くとの指摘を受け、`MARGIN_MAX_AGE_DAYS = 14` を導入。週末残高が14日より古い日は欠測（null）として線を途切れさせ、ツールチップからも除外。描画は欠測でstroke→再開でbeginPathのセグメント分割方式。ハーネス（中間9週を欠測にした合成データ）で線の分断・再開とギャップ区間のツールチップ非表示を目視確認。
  - **同日追記3（期間指定＋設定画面UI）**: 「2025年ごろまでで良い・設定画面から取り込みたい」との要望を受けて再設計。`iter_backfill(since)` ジェネレータ化（`--since YYYY[-MM-DD]`、イベント: start/week/week_error/done）→ `chat_server.py` に `POST /margin/backfill`（SSE、embedding系と同じパターン）→ 設定 > データタブに期間select（1年/2年/3年/全期間、既定2年）＋「過去分を取り込む」ボタン＋進捗表示（renderer-settings.js の既存 `streamPost` を再利用）。隔離uvicorn(:8021)で SSEイベント列・再実行で candidates 0（冪等）・不正since のerrorイベントをE2E確認。実データは全期間バックフィルを途中停止して2015〜16年分（19週）を削除し、`--since 2024` で2024年以降（Waybackにある18週）を投入し直した。設定画面UIの実画面表示は未確認。
...
- **2026-07-16: 価格帯別出来高をグラデーション表示に変更**。
  - 横棒のアルファ値を出来高比例に（`0.08 + ratio * 0.4`、ratio = ビン出来高 / 最大ビン出来高）。最大の価格帯が最も明るく、少ない帯は暗い。色相は従来の青（96, 165, 250）のまま。
  - 描画順をローソク足の前（背面）に移動。従来はローソク足の上に半透明バーが重なっていた。
...
- **2026-07-16: 株価チャートのタイムマシン表示（終端日付のスクラブ）を追加**。
  ...
  - 日付の強調: 見出しの日付はピル型バッジ（`.review-chart-date`、過去表示中は --warn 背景）。チャート内への日付描画は一度実装したが不要とのことで削除。
  - 実画面での操作感は未確認。
