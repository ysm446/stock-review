...
- **2026-07-19: 株価チャートに信用残（信用買い残・売り残）を追加**。
  ...
  - **同日追記11（指標カードの余白削減）**: `.review-lower-scroll` を `container-type: inline-size` にし、`@container (min-width: 860px)` で `.review-grid-layout` を3カラム＋財務サマリー `span 2` に（狭いときは従来の2カラム）。後方の `.review-financial-panel { grid-column: auto }` が span 2 を打ち消していたため削除。`.review-kv-row` はラベル列を固定110pxに、`.review-kv-value` を右端揃え→左揃えに変更してラベルと値の間延びを解消。実CSSハーネス（1400px窓、コンテナ950px/640px）で3カラム＋span2と2カラムフォールバックを目視確認。≤1180pxのメディアクエリ（2カラム強制）はコンテナ幅860px未満となるため実質衝突しない。
